...
task = Task_MM1(vis_noise=vis_noise, mem_noise=mem_noise, N_batch=N_trainbatch, 
                K=K, dat=tParams_new, dat_inds=train_inds)
...
train_params['training_weights_path'] = f'./{folder}/weights/{name}' 
train_params['loss_epoch'] = 5
train_params['save_training_weights_epoch'] = 15

losses, trainTime, initTime = model.train(task, train_params)
